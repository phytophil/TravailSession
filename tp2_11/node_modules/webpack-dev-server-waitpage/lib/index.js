 ...
 * @property ignore {string|RegExp|function|Array}
 */
...
let pluginWasCreatedExternally = false;
...
const webpackDevServerWaitpage = (server, options) => {
  if (
    ...
    throw new Error(
      `webpack-dev-server's server argument must be supplied as first parameter (or is it missing server.middleware.context.compiler)`
    );

  /** @type {WebpackDevServerWaitpageOptions} */
  options = Object.assign({}, defaultOptions, options);

  if (!pluginWasCreatedExternally) {
    const compilers = server.middleware.context.compiler.compilers || [
      server.middleware.context.compiler,
    ];
  ...
  return async (req, res, next) => {
    ...
    if (
      ...
      ) || // request is not a browser GET
      isIgnored(options.ignore, req)
    )
      // request was not ignored somehow
      return await next();
...
let count = 0;
webpackDevServerWaitpage.plugin = (options) => {
  pluginWasCreatedExternally = true;
  const i = count;
  count++;
